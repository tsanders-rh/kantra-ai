# kantra-ai Documentation

This directory contains all design documents and guides for kantra-ai.

## 📖 User Guides

Located in [`guides/`](./guides/):

- **[QUICKSTART.md](./guides/QUICKSTART.md)** - Get started quickly with basic usage examples
- **[WORKFLOW.md](./WORKFLOW.md)** - Complete end-to-end migration workflow from Konveyor analysis to manual development
- **[PROMPT_CUSTOMIZATION.md](./guides/PROMPT_CUSTOMIZATION.md)** - Customize AI prompts for your migration scenarios
- **[TESTING.md](./guides/TESTING.md)** - Comprehensive testing instructions and troubleshooting
- **[PR-TESTING-GUIDE.md](./guides/PR-TESTING-GUIDE.md)** - Guide for testing GitHub PR creation
- **[CONTRIBUTING.md](./guides/CONTRIBUTING.md)** - How to contribute to the project
- **[VALIDATION.md](./guides/VALIDATION.md)** - Validation and verification strategies

## 🏗️ Design Documents

Located in [`design/`](./design/):

- **[DESIGN.md](./design/DESIGN.md)** - Overall architecture and design decisions
- **[PLANNING_WORKFLOW_DESIGN.md](./design/PLANNING_WORKFLOW_DESIGN.md)** - Detailed design for the plan → execute workflow
- **[BATCH_PROCESSING_DESIGN.md](./design/BATCH_PROCESSING_DESIGN.md)** - Batch processing optimization design (50-80% cost reduction)

## 📋 Project Planning

- **[NEXT_STEPS.md](./NEXT_STEPS.md)** - Roadmap and future enhancements

## 🚀 Quick Links

- **Main Documentation**: [README.md](../README.md) in project root
- **Examples**: [examples/](../examples/) directory
- **Source Code**: [pkg/](../pkg/) directory

## 📚 Documentation by Topic

### Getting Started
1. Start with the main [README.md](../README.md)
2. Read [QUICKSTART.md](./guides/QUICKSTART.md) for basic usage
3. Review [WORKFLOW.md](./WORKFLOW.md) to understand the complete end-to-end workflow
4. Review [examples/](../examples/) for real-world scenarios

### Advanced Features
1. [PROMPT_CUSTOMIZATION.md](./guides/PROMPT_CUSTOMIZATION.md) - Customize AI prompts per language/technology
2. [PLANNING_WORKFLOW_DESIGN.md](./design/PLANNING_WORKFLOW_DESIGN.md) - Large-scale migration planning
3. [BATCH_PROCESSING_DESIGN.md](./design/BATCH_PROCESSING_DESIGN.md) - Cost/performance optimization

### Understanding Architecture
1. [DESIGN.md](./design/DESIGN.md) - Overall architecture
2. [PLANNING_WORKFLOW_DESIGN.md](./design/PLANNING_WORKFLOW_DESIGN.md) - Plan/execute workflow
3. [BATCH_PROCESSING_DESIGN.md](./design/BATCH_PROCESSING_DESIGN.md) - Performance optimizations

### Contributing
1. [CONTRIBUTING.md](./guides/CONTRIBUTING.md) - Contribution guidelines
2. [DESIGN.md](./design/DESIGN.md) - Understand the codebase
3. [TESTING.md](./guides/TESTING.md) - Testing strategies

### Testing & Validation
1. [TESTING.md](./guides/TESTING.md) - General testing guide
2. [PR-TESTING-GUIDE.md](./guides/PR-TESTING-GUIDE.md) - PR creation testing
3. [VALIDATION.md](./guides/VALIDATION.md) - Verification strategies
